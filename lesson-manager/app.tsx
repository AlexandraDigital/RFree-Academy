...
// Admin mode: ?admin=true in URL
const isAdmin = new URLSearchParams(window.location.search).get('admin') === 'true';

// Read user from localStorage (set by existing auth system)
...
function LessonDirectory() {
  ...
  const loadLessons = async () => {
    ...
    try {
      ...
      setError('Could not load lessons. Check your Worker URL in Settings.');
      console.error(e);
    } finally {
      setLoading(false);
    }
  ...
  const handleSaveLesson = async (data: Partial<Lesson>) => {
    ...
    try {
      if (editingLesson) {
        ...
        const created = await workerService.createLesson(data as any);
  ...
  const handleImport = async (file: File) => {
    try {
      ...
      for (const l of imported) {
        await workerService.createLesson(l as any);
      }
      await loadLessons();
    } catch {
  ...
  return (
    <div className="app-container">
      {isAdmin && (
        <div className="admin-mode-banner">🔒 Admin Mode</div>
      )}
