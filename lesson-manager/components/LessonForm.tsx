...
export const LessonForm: React.FC<LessonFormProps> = ({
  ...
  useEffect(() => {
    if (lesson) {
      ...
      setPublishedDate(lesson.published_date || new Date().toISOString().split('T')[0]);
      setGithubUrl(lesson.github_url || '');
    }
  }, [lesson]);

  const isOwner = !lesson || userRole === 'admin' || lesson.teacher_email === userEmail;

  const validate = () => {
    const errs: string[] = [];
    if (!name.trim()) errs.push('Lesson name is required');
    if (!videoUrl.trim()) errs.push('YouTube URL is required');
    if (!videoId) errs.push('Could not detect a YouTube video ID — check the URL');
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            ...
            <input type="text" value={name} onChange={e => setName(e.target.value)}
              placeholder="e.g., Introduction to Budgeting" disabled={!isOwner} />
          </div>

          <div className="form-group">
            <label>YouTube URL *</label>
            <input type="url" value={videoUrl} onChange={e => setVideoUrl(e.target.value)}
              placeholder="https://youtube.com/watch?v=..." disabled={!isOwner} />
            {videoId && (
          ...
          <div className="form-group">
            ...
            <input type="date" value={publishedDate}
              onChange={e => setPublishedDate(e.target.value)} disabled={!isOwner} />
          </div>

          <div className="form-group">
            <label>🔒 Source File URL <span style={{ fontSize: '0.75rem', color: '#888' }}>(optional, hidden from students)</span></label>
          ...
          <div className="form-group">
            ...
            <textarea value={outline} onChange={e => setOutline(e.target.value)}
              placeholder="Key topics and lesson summary..." rows={5} disabled={!isOwner} />
          </div>
